...
window.adicionarAoCarrinho = function(id) { 
    let itemExistente = carrinho.find(p => p.id === id);
    if(itemExistente) { itemExistente.qtd = (itemExistente.qtd || 1) + 1; } 
    else { let prod = listaDeProdutos.find(p => p.id === id); carrinho.push({...prod, qtd: 1}); }
    salvarCarrinhoNoLocal(); mostrarNotificacao("Adicionado!", 'sucesso'); 
};
window.alterarQtdCarrinho = function(index, delta) { 
    carrinho[index].qtd = (carrinho[index].qtd || 1) + delta; 
...
window.tentarCopiarPix = function() { if(!clienteLogadoCpf && !validaCamposCheckout()) return mostrarNotificacao("Preencha seu cadastro ou faça login para liberar o PIX!", "erro"); navigator.clipboard.writeText(configLoja.pix).then(() => mostrarNotificacao("Chave PIX copiada!", 'sucesso')); };
window.tentarFinalizar = function(e) { if(!clienteLogadoCpf && !validaCamposCheckout()) { e.preventDefault(); mostrarNotificacao("Preencha seus dados ou faça login para enviar o pedido!", "erro"); } };
...
document.getElementById('checkout-form').addEventListener('submit', async function(e) {
    e.preventDefault(); const btn = document.getElementById('btn-finalizar-checkout'); btn.disabled=true; btn.innerText="⏳ Processando...";
    ...
    try {
        ...
        let itensString = carrinho.map(i=> `${i.qtd||1}x ${i.nome}`).join(", ");
        const dataH = new Date();
        await addDoc(collection(db, "pedidos"), { cliente: nome, cpf, cidade: document.getElementById('cliente-bairro').value, telefone: tel, endereco, observacao: document.getElementById('cliente-obs').value, itens: itensString, detalhes_itens: carrinho, total, data: dataH.toLocaleDateString('pt-BR'), hora: dataH.toLocaleTimeString('pt-BR'), timestamp: dataH.toISOString(), status: "Pendente" });
    ...
    carrinho = []; salvarCarrinhoNoLocal(); localStorage.removeItem('maribella_form'); toggleCart(); this.reset(); btn.disabled=false; btn.innerText="💾 Enviar Pedido";
});
...
window.loginRapidoCheckout = async function() {
    ...
    if (d.exists() && d.data().senha === senha) { clienteLogadoCpf = cpf; clienteLogadoDados = d.data(); localStorage.setItem('maribella_auth_cliente', JSON.stringify({cpf, senha})); atualizarHeaderLogado(); prepararCheckoutLogado(); mostrarNotificacao("Preenchido! Confirme e envie o pedido.", "sucesso"); } else { mostrarNotificacao("Não encontrados.", "erro"); }
};

// --- PAINEL DO CLIENTE (LOGIN E RECUPERAÇÃO) ---
...
document.getElementById('form-login-cliente').addEventListener('submit', async function(e) {
    ...
    if(d.exists() && d.data().senha === senha) { clienteLogadoCpf = cpf; clienteLogadoDados = d.data(); if(manter) localStorage.setItem('maribella_auth_cliente', JSON.stringify({cpf, senha})); atualizarHeaderLogado(); fecharLoginCliente(); abrirPainelCliente(d.data()); this.reset(); } else mostrarNotificacao("CPF/Senha incorretos.", "erro");
});
...
document.getElementById('form-recuperacao').addEventListener('submit', async function(e) {
    ...
    if(d.exists() && d.data().telefone === tel) { await updateDoc(docRef, {senha: novaSenha}); mostrarNotificacao("Senha alterada!", "sucesso"); fecharRecuperacao(); abrirLoginCliente(); this.reset(); } else { mostrarNotificacao("Dados não conferem.", "erro"); }
});
...
// --- HISTÓRICO, LIXEIRA E EDIÇÃO DO CLIENTE ---
window.carregarMeusPedidosPainel = async function(cpf) {
    ...
    snap.forEach(d => {
        ...
        if(p.cpf === cpf) {
            tem = true; meusPedidosSalvos.push(p);
            let cor = p.status === 'Aprovado' ? 'var(--success)' : p.status === 'Cancelado' ? '#e74c3c' : '#f39c12';
...
window.abrirEdicaoPedido = function(id) {
    pedidoEmEdicao = JSON.parse(JSON.stringify(meusPedidosSalvos.find(p => p.id === id))); // Cópia
    if(!pedidoEmEdicao.detalhes_itens) return mostrarNotificacao("Erro ao ler itens.", "erro");
    pedidoEmEdicao.detalhes_itens.forEach(i => i.qtd = i.qtd || 1); // Garante qtd
    renderizarEdicaoPedido();
...
function renderizarEdicaoPedido() {
    const lista = document.getElementById('lista-editar-itens'); lista.innerHTML = ""; let total = 0;
    if(pedidoEmEdicao.detalhes_itens.length === 0) lista.innerHTML = "<p style='color:red;'>O pedido ficará vazio (Será cancelado ao salvar).</p>";
    pedidoEmEdicao.detalhes_itens.forEach((item, index) => {
        total += parseFloat(item.preco) * item.qtd;
        lista.innerHTML += `<div style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #eee;">
            <div style="flex:1;"><strong>${item.nome}</strong><br><span style="color:#888;">R$ ${parseFloat(item.preco).toFixed(2)}</span></div>
            <div style="display:flex; align-items:center; gap:8px;"><button onclick="alterarQtdEdicao(${index}, -1)" style="border:none; background:#eee; padding:5px 10px; border-radius:5px; font-weight:bold;">-</button><span>${item.qtd}</span><button onclick="alterarQtdEdicao(${index}, 1)" style="border:none; background:#eee; padding:5px 10px; border-radius:5px; font-weight:bold;">+</button></div>
        </div>`;
    });
    document.getElementById('novo-total-edicao').innerText = total.toFixed(2);
}
window.alterarQtdEdicao = function(index, delta) {
    pedidoEmEdicao.detalhes_itens[index].qtd += delta;
    if(pedidoEmEdicao.detalhes_itens[index].qtd <= 0) pedidoEmEdicao.detalhes_itens.splice(index, 1);
    renderizarEdicaoPedido();
};
window.salvarEdicaoPedido = async function() {
    ...
    let total = 0; pedidoEmEdicao.detalhes_itens.forEach(i => total += parseFloat(i.preco) * i.qtd);
    let strItens = pedidoEmEdicao.detalhes_itens.map(i => `${i.qtd}x ${i.nome}`).join(", ");
    try { await updateDoc(doc(db, "pedidos", pedidoEmEdicao.id), { detalhes_itens: pedidoEmEdicao.detalhes_itens, itens: strItens, total: total.toFixed(2) }); mostrarNotificacao("Pedido atualizado!", "sucesso"); fecharEdicaoPedido(); carregarMeusPedidosPainel(clienteLogadoCpf); } catch(e) { mostrarNotificacao("Erro ao salvar.", "erro"); }
};
...
document.getElementById('form-login-admin').addEventListener('submit', async function(e) { e.preventDefault(); try { await signInWithEmailAndPassword(auth, document.getElementById('admin-email').value, document.getElementById('admin-senha').value); mostrarNotificacao("Acesso Liberado!", "sucesso"); fecharLoginAdmin(); document.getElementById('admin-dashboard').classList.remove('hidden'); carregarListaAdminPedidos(); } catch(e) { mostrarNotificacao("Erro!", "erro"); } });
...
window.filtrarPedidosAdmin = function() {
    const termo = document.getElementById('busca-pedido').value.toLowerCase(); const filtro = document.getElementById('filtro-status').value; const lista = document.getElementById('lista-admin-pedidos'); lista.innerHTML = "";
...
window.filtrarProdutosAdmin = function() { const termo = document.getElementById('busca-produto-admin').value.toLowerCase(); const lista = document.getElementById('lista-admin-produtos-cadastrados'); lista.innerHTML = ""; let filtrados = todosProdutosAdmin.filter(p => p.nome.toLowerCase().includes(termo)); if(filtrados.length === 0) lista.innerHTML = "<p>Nenhum produto encontrado.</p>"; filtrados.forEach(p => { lista.innerHTML += `<div class="admin-card" style="display:flex; align-items:center; gap:15px; padding:10px; border-bottom:1px solid #eee;"><img src="${p.imagem}" style="width:60px; height:60px; border-radius:8px; object-fit:cover; border: 1px solid #ccc;"><div style="flex:1;"><strong>${p.nome}</strong><br><span style="color:var(--secondary); font-weight:bold;">R$ ${p.preco}</span></div><button onclick="editarProdutoAdmin('${p.id}', '${p.nome}', ${p.preco}, '${p.tamanho}', '${p.material}')" class="btn-icon" style="background:var(--secondary); color:white;">✏️ Editar</button></div>`; }); }
...
window.filtrarClientesAdmin = function() { const termo = document.getElementById('busca-cliente-admin').value.toLowerCase(); const lista = document.getElementById('lista-admin-clientes'); lista.innerHTML = ""; let filtrados = todosClientesAdmin.filter(c => c.nome.toLowerCase().includes(termo) || c.telefone.includes(termo)); if(filtrados.length === 0) lista.innerHTML = "<p>Nenhum cliente encontrado.</p>"; filtrados.forEach(c => { let telLimpo = c.telefone.replace(/\D/g, ''); lista.innerHTML += `<div class="admin-card" style="border-left-color: #2ecc71; display:flex; justify-content:space-between; align-items:center;"><div><strong>${c.nome}</strong><br><span>${c.telefone}</span></div><a href="https://wa.me/55${telLimpo}" target="_blank" style="background:#25D366; color:white; padding:10px; border-radius:50%; text-decoration:none;">💬</a></div>`; }); }
