...
// Notificações: Agora a "Info" (Venda Fictícia) tem um X e fica mais tempo
window.mostrarNotificacao = (msg, t = 'sucesso', fechavel = false) => { 
    const toast = document.getElementById('toast-notificacao'); 
    let icone = t==='erro'?"❌":t==='info'?"ℹ️":"✅"; 
    let fecharBtn = fechavel ? `<span onclick="fecharToast()" style="margin-left:15px;cursor:pointer;font-size:1.2rem;">&times;</span>` : '';
    toast.innerHTML = `<span style="flex:1;">${icone} ${msg}</span> ${fecharBtn}`; 
...
// A função precisa estar 100% no window para não dar erro
...
setInterval(() => {
    ...
    // Manda como Info e com 'fechavel = true' (Mostra o X e não some sozinho)
    mostrarNotificacao(`🛍️ ${n} de ${c} comprou R$ ${v}!`, 'info', true);
    setTimeout(window.fecharToast, 8000); // Some sozinho depois de 8s se não fechar
...
window.salvarConfiguracoes = async (e) => { e.preventDefault(); const btn = document.getElementById('btn-salvar-config'); btn.innerText = "⏳..."; try { configLoja.pix = document.getElementById('config-pix').value; configLoja.telefone = document.getElementById('config-telefone').value; await setDoc(doc(db, "config", "loja"), configLoja); document.getElementById('pix-display').innerText = configLoja.pix; mostrarNotificacao("Salvo!", "sucesso"); } catch(err) {} btn.innerText = "💾 Atualizar Dados"; };

// --- VITRINE LADO A LADO E ROLAGEM DE 1 EM 1 ---
...
function criarSecaoCarrossel(titulo, produtos, containerMaster) {
    ...
    produtos.forEach(p => {
        ...
        carrossel.innerHTML += `<div class="card"><img src="${p.imagem}" onclick="abrirLightbox('${p.imagem}')"><div class="card-info"><div><h3>${p.nome}</h3><p><strong>Tam:</strong> ${p.tamanho}</p><p style="font-size:0.75rem;">${estoqueTxt}</p><p class="preco">R$ ${parseFloat(p.preco).toFixed(2)}</p></div>${btnStatus}</div></div>`;
    });
    containerMaster.appendChild(section);

    // Auto Scroll exato (1 card por vez)
    let autoScroll = setInterval(() => {
        if(!carrossel.querySelector('.card')) return;
        let cardWidth = carrossel.querySelector('.card').clientWidth + 10; // Largura + Gap
...
window.carregarMeusPedidosPainel = async (cpf) => {
    const lista = document.getElementById('lista-meus-pedidos'); lista.innerHTML = "⏳...";
...
window.excluirProdutoAdmin = async (id) => { if(confirm("🗑️ Deseja realmente excluir este produto? Essa ação não pode ser desfeita.")) { await deleteDoc(doc(db, "produtos", id)); mostrarNotificacao("Excluído!", "info"); carregarListaAdminProdutosEditar(); carregarProdutosDoBanco(); } };
...
// --- GERAR DADOS FICTÍCIOS Ricos (Item 4) ---
window.gerarDadosDeExemplo = async () => {
    mostrarNotificacao("Gerando Lançamentos, Vestidos, Shorts e Blusas... Aguarde!", "info");
    const prods = [
        ...
        {nome: "Vestido Tule Mágico", preco: 120.00, estoque: 5, categoria: "Vestido", tamanho: "4, 6, 8", cores: "Branco", material: "Tule", imagem: "https://images.unsplash.com/photo-1543165365-07232ed12fad?w=500"},
        {nome: "Lançamento Conjunto Verão", preco: 119.90, estoque: 15, categoria: "Lançamento", tamanho: "4, 6, 8", cores: "Estampado", material: "Viscose", imagem: "https://images.unsplash.com/photo-1519457431-44ccd64a579b?w=500"},
        {nome: "Lançamento Macaquito Lindo", preco: 95.00, estoque: 8, categoria: "Lançamento", tamanho: "1, 2, 3", cores: "Azul", material: "Linho", imagem: "https://images.unsplash.com/photo-1519241047957-be31d7379a5d?w=500"},
        ...
        {nome: "Saia Rodada Brilho", preco: 55.00, estoque: 12, categoria: "Short/Calça/Saia", tamanho: "4, 6", cores: "Prata", material: "Paetê", imagem: "https://plus.unsplash.com/premium_photo-1682089874677-3eee554ffe38?w=500"},
        {nome: "Blusa Babado Luxo", preco: 45.00, estoque: 20, categoria: "Blusa/Cropped/Body", tamanho: "2, 4, 6, 8", cores: "Branca", material: "Algodão", imagem: "https://images.unsplash.com/photo-1618828665011-0abd973f7bb8?w=500"},
        {nome: "Cropped Tie Dye", preco: 39.90, estoque: 10, categoria: "Blusa/Cropped/Body", tamanho: "8, 10, 12", cores: "Colorido", material: "Malha", imagem: "https://images.unsplash.com/photo-1503342394128-c104d54dba01?w=500"}
    ];
    for(let p of prods) await addDoc(collection(db, "produtos"), p);
    
    await setDoc(doc(db, "clientes", "11111111111"), {nome: "Carla Mendes (Exemplo)", cpf: "111.111.111-11", telefone: "(81) 99999-9999", senha: "123", cep: "50000-000", rua: "Av Principal", numero: "10", bairro: "Boa Viagem", estado: "PE"});
    await addDoc(collection(db, "pedidos"), {cliente: "Carla Mendes (Exemplo)", cpf: "11111111111", cidade: "Boa Viagem", telefone: "(81) 99999-9999", endereco: "Av Principal, 10 - Boa Viagem", observacao: "Deixar na portaria", itens: "1x Vestido Floral Princesa", detalhes_itens: [], total: "89.90", data: new Date().toLocaleDateString('pt-BR'), hora: new Date().toLocaleTimeString('pt-BR'), timestamp: new Date().toISOString(), status: "Pendente"});
    
    mostrarNotificacao("Pronto! Recarregando a loja em 2s...", "sucesso");
