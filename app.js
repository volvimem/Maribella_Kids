...
let configLoja = { pix: "", telefone: "5581999999999", aviso: "Enviamos para todo o Brasil.", instagram: "", endereco: "" };
...
setTimeout(dispararVendaFalsa, Math.random() * 30000 + 30000); // 1ª depois de 30s

// PESSOAS ONLINE (Inteligente por Hora)
function atualizarPessoasOnline() {
    let el = document.getElementById('online-count');
    if(!el) return;
    let hora = new Date().getHours();
    let minBase, maxBase;
    if (hora >= 9 && hora <= 14) { minBase = 2000; maxBase = 4734; } // Manhã/Almoço (Pico)
    else if (hora >= 18 && hora <= 22) { minBase = 2500; maxBase = 4734; } // Noite (Pico)
    else if (hora >= 1 && hora <= 6) { minBase = 126; maxBase = 500; } // Madrugada (Baixo)
    else { minBase = 800; maxBase = 2000; } // Normal
    ...
    let variacao = Math.floor(Math.random() * 60) - 25;
    let novoValor = atual + variacao;
...
// --- MENU LATERAL (Corrigido e Blindado no Window) ---
window.toggleMenu = () => { 
...
function renderizarVitrinesCategorias(lista, tituloUnico = null) {
    ...
    if(tituloUnico) { criarSecaoCarrossel(tituloUnico, lista, container, indexFila); } 
        ...
        let outros = lista.filter(p => !['Vestido','Short/Calça/Saia','Blusa/Cropped/Body','Conjunto'].includes(p.categoria) && !p.lancamento);
        ...
        if(outros.length > 0) criarSecaoCarrossel('Mais Opções 🎀', outros, container, indexFila++);
    }
}
...
window.adicionarAoCarrinho = (id) => { let prod = listaDeProdutos.find(p => p.id === id); let itemEx = carrinho.find(p => p.id === id); let qtdAtual = itemEx ? itemEx.qtd : 0; if(qtdAtual + 1 > prod.estoque) return mostrarNotificacao("Sem estoque suficiente!", 'erro'); if(itemEx) itemEx.qtd++; else carrinho.push({...prod, qtd: 1}); salvarCarrinhoNoLocal(); window.mostrarNotificacao("Adicionado!", 'sucesso'); };
...
window.buscarCep = async (cepV, pref) => { let cep = cepV.replace(/\D/g, ''); if (cep.length === 8) { try { let data = await (await fetch(`https://viacep.com.br/ws/${cep}/json/`)).json(); if (!data.erro) { document.getElementById(`${pref}-rua`).value = data.logradouro; document.getElementById(`${pref}-bairro`).value = data.bairro; document.getElementById(`${pref}-estado`).value = data.uf; if(pref==='cliente') window.salvarFormulario(); } } catch(err) {} } };
...
window.cancelarMeuPedido = async (id) => { if(confirm("Deseja cancelar este pedido?")) { await updateDoc(doc(db, "pedidos", id), { status: "Cancelado" }); window.carregarMeusPedidosPainel(clienteLogadoCpf); } };
...
window.sairCliente = () => { localStorage.removeItem('maribella_auth_cliente'); clienteLogadoCpf = null; clienteLogadoDados = null; atualizarHeaderLogado(); window.fecharPerfil(); window.mostrarNotificacao("Sessão encerrada.", "info"); };
...
window.filtrarPedidosAdmin = () => {
    ...
    res.forEach(p => { 
        let btnAprovar = p.status === 'Pendente' ? `<button onclick="window.aprovarPedido('${p.id}')" style="background:#2ecc71; color:white; border:none; padding:8px; border-radius:5px;">✅ Aprovar</button>` : '';
        let btnVoltar = p.status !== 'Pendente' ? `<button onclick="window.voltarPendentePedido('${p.id}')" style="background:#f39c12; color:white; border:none; padding:8px; border-radius:5px;">↩️ Pendente</button>` : '';
        let btnExcluir = `<button onclick="window.excluirPedidoAdmin('${p.id}')" style="background:#e74c3c; color:white; border:none; padding:8px; border-radius:5px;">🗑️ Excluir</button>`;
...
window.salvarProdutoAdmin = async (e) => { 
    e.preventDefault(); const btn = document.getElementById('btn-salvar-produto'); const img = document.getElementById('add-imagem-file').files[0]; const id = document.getElementById('edit-produto-id').value; btn.innerText = "⏳..."; btn.disabled = true; 
    try { 
        ...
        if (id) { await updateDoc(doc(db, "produtos", id), pData); } else { if(!urlFoto) return window.mostrarNotificacao("Foto obrigatória!","erro"); await addDoc(collection(db, "produtos"), pData); } 
...
window.excluirProdutoAdmin = async (id) => { if(confirm("🗑️ ATENÇÃO: Deseja apagar essa peça?")) { await deleteDoc(doc(db, "produtos", id)); window.mostrarNotificacao("Peça apagada!", "info"); carregarListaAdminProdutosEditar(); window.carregarProdutosDoBanco(); } };
...
window.gerarDadosDeExemplo = async () => {
    ...
    await addDoc(collection(db, "pedidos"), {cliente: "Carla Mendes (Ex)", cpf: "11111111111", cidade: "Boa Viagem", telefone: "(81) 99999-9999", endereco: "Av Principal, 10 - Boa Viagem", observacao: "Deixar na portaria", itens: "1x Vestido Floral", detalhes_itens: [], total: "89.90", data: new Date().toLocaleDateString('pt-BR'), hora: new Date().toLocaleTimeString('pt-BR'), timestamp: new Date().toISOString(), status: "Pendente"});
